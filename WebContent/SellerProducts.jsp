...
<div id="header">
	...
	<div id="body">
		...
		<br>
				...
				<%
				List<Product> sellerplist =(List<Product>)request.getAttribute("sellerplist");
				...
				if (sellerplist!=null){ %>
					<center><table>
						...
						<%
						for (int i = 0; i < sellerplist.size(); i++) {
						%>
						<tr>
							<td><%=sellerplist.get(i).getProductname() %></td>
							<td><%=sellerplist.get(i).getDescription() %></td>
							<td><%=sellerplist.get(i).getQuantity() %></td>
							<td><%=sellerplist.get(i).getCompanyname() %></td>
							<td><%=sellerplist.get(i).getPrice() %></td>
							  <%authDAO_signup AD=new authDAO_signup();
								int productid=sellerplist.get(i).getProductid();
							%>
							<td>
